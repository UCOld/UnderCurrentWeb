...
UnderCurrent Web is a comprehensive web application that uses the UnderCurrent Core server to allow its users to interact with their game server instance in new ways.

## Get Started
1. **Initial Machine Setup**. First time running UnderCurrent Web? Then complete the [Initial Machine Setup](https://github.com/UnderCurrentTeam/UnderCurrentWeb#initial-machine-setup).
2. **Clone the project**. `git clone https://github.com/UnderCurrentTeam/UnderCurrentWeb.git`.
3. **Run the setup script**. `npm run setup`
4. **Run the app**. `npm start`
This will run the automated build process, start up a webserver, and open the application in your default browser. When doing development, this command will continue watching all your files. Every time you hit save the code is rebuilt, linting runs, and tests run automatically. Note: There is an optional -s flag which enables silent mode that suppresses unnecessary messages during the build. 
...
1. Make sure you ran all steps in [Get started](https://github.com/UnderCurrentTeam/UnderCurrentWeb/blob/master/README.md#get-started) including the [initial machine setup](https://github.com/UnderCurrentTeam/UnderCurrentWeb#initial-machine-setup).
...
##Technologies
UnderCurrentWeb offers a rich development experience using the following technologies:
